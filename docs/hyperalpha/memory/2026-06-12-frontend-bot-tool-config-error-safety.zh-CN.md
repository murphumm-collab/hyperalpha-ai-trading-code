# 2026-06-12 Frontend Bot/Tool Config Error Safety Memory
...
- Hyper AI Bot Integration 连接失败不再渲染 raw backend detail、provider response、token validation error 或 network exception text
- Hyper AI Tool Config 保存失败不再渲染 raw `data.error`、`data.detail` 或 thrown/network exception message
- Hyper AI Tool Config 删除失败不再只返回裸 `Remove failed` 或 raw exception；失败路径统一映射到固定安全 UI label
- Source guard 覆盖 `formatBotIntegrationConnectError`、`formatToolConfigSaveError`、`formatToolConfigRemoveError`、`.json().catch(() => ({}))` 和 raw error 回归路径
- `ai_trading_v1_completion_audit.py --strict-local` 新增 status marker gate：`| AI Trading frontend bot/tool config error safety | Done |`

## 已跑验证

- `cd backend && uv run pytest tests/test_ai_trading_frontend_readiness_source.py -q -k "bot_and_tool_config_errors or model_config_modal_uses_safe_api_error_formatter"`：2 passed
- `cd backend && uv run pytest tests/test_ai_trading_v1_completion_audit.py -q -k "frontend_bot_tool_config_error_safety or frontend_model_config_error_safety or current_repo_completion_audit"`：3 passed
...
- 最新本地 mock handoff evidence：spec `#166`、signal event `#164`、handoff attempt `#162`、gateway response `mock_accepted`、`agent_sessions.total=152`、`handoff_attempts.total=162`
