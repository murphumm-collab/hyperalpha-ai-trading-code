...
- Legacy onboarding 不再提供 optional model setup 或 `/api/hyper-ai/profile/llm` 提交路径；所有 API key 配置都后置到主页面内完成。
- Completion audit 新增 marker：
  - `| AI Trading frontend onboarding API-key deferral | Done |`

## 已验证

- `cd backend && uv run pytest tests/test_ai_trading_frontend_readiness_source.py -q -k "onboarding_defers_api_key or model_config_is_nonblocking or onboarding_uses_safe_stream"` returned 3 passing tests.
