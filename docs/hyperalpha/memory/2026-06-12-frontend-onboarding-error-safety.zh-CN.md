...
- GitHub 上传：本分支当前已同步到 origin/codex/ai-agent-multitenant-foundation
- 分支纪律：只 push feature branch，不 merge
...
- Legacy Hyper AI onboarding 已不再渲染进入前的 DeepSeek/Qwen 模型/API-key 保存表单；模型配置错误安全由后续页面内的 LLM Config modal 覆盖。
- Onboarding 不再把 raw backend detail、provider response、token validation error、network exception text 或 API key 相关错误直接渲染给 To C 用户，也不再提交 `/api/hyper-ai/profile/llm`。
- Onboarding chat stream `error` chunk 不再渲染 `chunk.data?.message`，统一抛固定 `Stream error` label，由 UI 显示通用失败提示
- Source guard 覆盖 legacy onboarding 无 `formatAiTradingModelConfigApiError` / `api_key` / `value={apiKey}` / password field / `/api/hyper-ai/profile/llm`，以及 raw `chunk.data?.message` 回归路径
- `ai_trading_v1_completion_audit.py --strict-local` 新增 status marker gate：`| AI Trading frontend onboarding error safety | Done |`

## 已跑验证

- `cd backend && uv run pytest tests/test_ai_trading_frontend_readiness_source.py -q -k "onboarding_uses_safe_stream or model_config_modal_uses_safe_api_error_formatter"`：2 passed
