...
- `backend/scripts/ai_trading_v1_completion_audit.py`：
  ...
  - Evidence `artifact_refs` 现在每项最多 5 个、每条最多 300 字符，防止用引用字段夹带原始日志或模型输出。
  ...
  - Evidence 文件路径现在必须在代码仓库外；repo 内的 production evidence JSON 只能作为模板/负向检查，不能解锁 live ready。
  - Evidence root `notes` 现在必须是最多 12 条、每条最多 300 字符的短文本列表，防止用允许字段夹带原始日志、模型输出、trace 或运维 dump。
...
- `docs/hyperalpha/ai-trading-v1-production-evidence.template.json`：
  ...
  - 模板 notes 明确 `artifact_refs` 必须非空，只能使用 `https://`、`ops://`、`lark://`、`notion://`，每项最多 5 个、每条最多 300 字符，不能嵌入 credentials 或指向 localhost/private-network URL。
  ...
  - 模板 notes 明确填好的 production evidence 文件必须复制到代码仓库外或私有 ops evidence 位置，repo-local evidence 不能解锁 live ready。
  - 模板 notes 明确 root `notes` 只能是有界短文本列表，不能用于 raw logs/model output/traces/dumps。
...
- `backend/tests/test_ai_trading_v1_completion_audit.py`：
  ...
  - 覆盖过长 artifact ref、过多 artifact refs 不能 live ready，避免用引用字段夹带 raw dumps。
  ...
  - 覆盖 repo-local production evidence 文件不能 live ready，避免把生产验收材料当作代码提交。
  - 覆盖 root `notes` 非 list、过长 note、过多 notes 不能 live ready，避免用允许字段夹带 raw dumps。
...
- `cd backend && uv run pytest tests/test_ai_trading_v1_completion_audit.py -q`：14 passed。
...
- 临时 artifact-ref bounds CLI 检查：过长 ref 和超过 5 个 refs 会按预期阻断，不允许 live ready。
...
- 临时 path-safety CLI 检查：repo 内完整 accepted evidence 文件会按预期阻断，不允许 live ready。
- 临时 note-safety CLI 检查：root `notes` 非 list、过长 note、过多 notes 会按预期阻断，不允许 live ready。
- `cd backend && uv run pytest tests/test_ai_trading_v1_completion_audit.py tests/test_ai_trading_env_check.py tests/test_ai_trading_live_stack_acceptance.py tests/test_ai_trading_model_adjust_live_acceptance.py tests/test_ai_trading_production_readiness_check.py tests/test_ai_trading_production_readiness_api.py tests/test_ai_trading_routes.py tests/test_ai_trading_mock_gateway.py tests/test_ai_trading_production_handoff_check.py -q`：77 passed，5 个既有 UTC deprecation warnings。
- `scripts/local-dev/run_ai_trading_v1_local_acceptance.sh --confirm-local-mock-handoff`：passed；一键本地验收覆盖 backend compile、77 条 AI Trading 回归、API smoke、live model-adjust 默认阻断、默认 production handoff/readiness/DB-audit blockers、local V1 completion boundary audit、production completion boundary expected blocker、production evidence template expected blocker、frontend build、runtime readiness 和 live local mock handoff。最新证据为 strategy spec `#55`、signal event `#53`、agent sessions `40`、handoff attempts `51`、gateway response `mock_accepted`、model-adjust blocker `model_profile_not_configured`。
...
- Evidence 文件不能包含 API keys、bearer tokens、DB URLs、private keys、raw Authorization headers、用户 secret；timestamp 必须带时区且顺序一致；artifact refs 也不能指向本地/私网 URL、嵌入 credentials、超长或超量；root/item 不能带模板外的任意字段；`items` 不能包含未知验收项 ID；`validated_by` 和 `evidence_summary` 不能是 placeholder、过短形式化文本或过长原始输出；填好的 evidence 文件必须在代码仓库外或私有 ops 位置。
