...
- 新增一键本地 V1 验收 runner：`scripts/local-dev/run_ai_trading_v1_local_acceptance.sh --confirm-local-mock-handoff`，显式确认后才会跑 live mock handoff；后续增强 env checker 结构化解析 `runtime_gateway` 并要求 `target_kind=local_mock` 且无 runtime blockers；当前也纳入 live DeepSeek/Qwen model-adjust 默认阻断、默认生产 handoff gate、生产总 readiness gate 阻断检查和 admin readiness API 回归；model-adjust session-context redaction 后最新完整通过 backend compile、57 条 AI Trading 回归、API smoke、默认生产 gate 阻断、默认生产总 readiness 阻断、frontend build、runtime readiness、live local mock handoff，最新证据为 spec `#25`、signal event `#23`、runtime `target_kind=local_mock`、`agent_sessions.total=10`、`model_adjustment.ready=false` 且 blocker 为 `model_profile_not_configured`；不带确认参数时已验证会拒绝运行并退出 2。
...
- Model-adjust session context 后新增验证：`cd backend && uv run python -m py_compile api/ai_trading_routes.py services/ai_trading_strategy_spec_service.py tests/test_ai_trading_routes.py` 通过；`cd backend && uv run pytest tests/test_ai_trading_routes.py -q` 返回 34 条全绿，覆盖未保存草案和已保存 spec 的敏感 session summary 都会被替换为 `[redacted_sensitive_context]` 且不进入 prompt/response；合并 AI Trading 回归返回 57 条全绿；`cd frontend && npm run build` 通过；`cd backend && uv run python scripts/ai_trading_v1_acceptance_smoke.py` 返回 `success=true`；一键本地 V1 验收 runner 最新证据为 spec `#25`、signal event `#23`、`agent_sessions.total=10`。
- Model-adjust session context 后已重跑 `scripts/local-dev/install_launch_agent.sh` 同步 runtime mirror；第一次 strict readiness 命中 backend 冷启动，12 秒后复查 `cd backend && uv run python scripts/ai_trading_v1_env_check.py --strict` 返回 `ready=true`；最新一键验收后 runtime totals 为 strategy specs `25`、signal events `23`、agent sessions `10`。
