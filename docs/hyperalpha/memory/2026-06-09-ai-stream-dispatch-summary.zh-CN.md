...
- 未保存草案 model-adjust 如果传入 `agent_session_id`，后端会先校验它是当前用户 active AI Trading agent session；跨用户、不存在或 archived session 会在读取模型配置前失败，避免伪造别人的 session context 标签。
...
- Archived agent session 现在也会阻止已保存 strategy spec 继续 natural-language adjust、DeepSeek/Qwen model-adjust、approval、backtest attach/latest/preflight、signal preview/event creation 和 confirmed handoff；model-adjust 在 archived gate 失败时不会读取模型配置或发模型请求，signal-event eligibility / handoff attempt 会暴露非敏感 `agent_session_archived` blocker，历史 detail/audit 仍可读。
...
- 新增一键本地 V1 验收 runner：`scripts/local-dev/run_ai_trading_v1_local_acceptance.sh --confirm-local-mock-handoff`，显式确认后才会跑 live mock handoff；后续增强 env checker 结构化解析 `runtime_gateway` 并要求 `target_kind=local_mock` 且无 runtime blockers；当前也纳入 live DeepSeek/Qwen model-adjust 默认阻断、默认生产 handoff gate、生产总 readiness gate 阻断检查和 admin readiness API 回归；model-adjust session-context ownership/redaction 后最新完整通过 backend compile、58 条 AI Trading 回归、API smoke、默认生产 gate 阻断、默认生产总 readiness 阻断、frontend build、runtime readiness、live local mock handoff，最新证据为 spec `#27`、signal event `#25`、runtime `target_kind=local_mock`、`agent_sessions.total=12`、`model_adjustment.ready=false` 且 blocker 为 `model_profile_not_configured`；不带确认参数时已验证会拒绝运行并退出 2。
...
- Model-adjust session context 后新增验证：`cd backend && uv run python -m py_compile api/ai_trading_routes.py services/ai_trading_strategy_spec_service.py tests/test_ai_trading_routes.py` 通过；`cd backend && uv run pytest tests/test_ai_trading_routes.py -q` 返回 35 条全绿，覆盖未保存草案和已保存 spec 的敏感 session summary 都会被替换为 `[redacted_sensitive_context]` 且不进入 prompt/response，也覆盖未保存草案跨用户/archived session 在读取模型配置前失败，以及 archived session 下已保存 spec 不能继续 natural/model-adjust；合并 AI Trading 回归返回 58 条全绿；`cd frontend && npm run build` 通过；`cd backend && uv run python scripts/ai_trading_v1_acceptance_smoke.py` 返回 `success=true`；一键本地 V1 验收 runner 证据为 spec `#27`、signal event `#25`、`agent_sessions.total=12`。
- Archived-session action gate 后新增验证：`cd backend && uv run python -m py_compile services/ai_trading_strategy_spec_service.py api/ai_trading_routes.py tests/test_ai_trading_routes.py` 通过；`cd backend && uv run pytest tests/test_ai_trading_routes.py -q` 返回 35 条全绿，覆盖 archived session 阻止 approval、backtest summary/result/latest/preflight、signal preview/event creation、confirmed handoff，并在 signal-event eligibility / handoff attempt 中返回 `agent_session_archived` blocker；合并 AI Trading 回归返回 58 条全绿；一键本地 V1 验收 runner 最新证据为 spec `#29`、signal event `#27`、`agent_sessions.total=14`。
- Archived-session action gate 后已重跑 `scripts/local-dev/install_launch_agent.sh` 同步 runtime mirror；`cd backend && uv run python scripts/ai_trading_v1_env_check.py --strict` 返回 `ready=true`；最新一键验收后 runtime totals 为 strategy specs `29`、signal events `27`、agent sessions `14`。
- Archived-session blocker UI 后新增验证：Hyper AI `signalBlockerLabel` 现在把 `agent_session_archived` 显示成 `Agent session archived`，用于 signal handoff title、recent signal row、session detail 和 handoff attempt review text；`cd frontend && npm run build` 通过，剩余为既有 browserslist/baseline/chunk-size warnings。
- Archived-session frontend action gate 后新增验证：Hyper AI 当前策略卡和 recent spec rows 会在 selected/owning session archived 时禁用 save-to-archived-session、approve、persisted natural/model-adjust、backtest attach/latest/preflight/run、signal preview 等写动作，并显示 inline `Agent session archived` warning；inspect spec/backtest evidence、session detail、handoff attempt audit 等读操作仍可用；`cd frontend && npm run build`、`scripts/local-dev/install_launch_agent.sh`、`cd backend && uv run python scripts/ai_trading_v1_env_check.py --strict` 均通过。
- Agent-session status payload 后新增验证：strategy spec、signal event、handoff attempt API response 的 `agent_session` 现在会在 current-user DB 上下文下返回 `status`，前端 archived-session gate 优先用 record payload 的 `agent_session.status`，只有缺 status 时才 fallback 到已加载 session list；`cd backend && uv run python -m py_compile services/ai_trading_strategy_spec_service.py api/ai_trading_routes.py tests/test_ai_trading_routes.py` 通过；`cd backend && uv run pytest tests/test_ai_trading_routes.py -q` 返回 35 条全绿，覆盖 active/archived status payload；合并 AI Trading 回归返回 58 条全绿；`cd frontend && npm run build` 通过；重跑 `scripts/local-dev/install_launch_agent.sh` 后 strict readiness 冷启动重试返回 `ready=true`；一键本地 V1 验收 runner 最新证据为 spec `#31`、signal event `#29`、`agent_sessions.total=16`、runtime `target_kind=local_mock`。
- Failed-handoff retry audit 后新增验证：gateway handoff 第一次失败会保留 review_candidate/failed、写 failed attempt、脱敏错误；随后必须再次显式确认并重新通过 eligibility 才能 retry，成功后 event 变成 submitted、error_message 清空、attempt history 按最新 submitted 再 failed 排序，且成功/失败 gateway response 都只保留白名单字段；前端 failed 且 eligible 的 signal badge 显示 `Retry ready`，tooltip 显示 `Retry handoff`；`cd backend && uv run pytest tests/test_ai_trading_routes.py -q` 返回 35 条全绿，合并 AI Trading 回归返回 58 条全绿，`cd frontend && npm run build` 通过；一键本地 V1 验收 runner 最新证据为 spec `#32`、signal event `#30`、`agent_sessions.total=17`、runtime `target_kind=local_mock`。
...
- 新增本地自启动脚本：`scripts/local-dev/ai_trading_local_supervisor.sh`、`install_launch_agent.sh`、`uninstall_launch_agent.sh` 和 LaunchAgent plist。安装时会把当前项目同步到 `/Users/mo/Library/Application Support/HyperAlpha/runtime/hyperalpha-ai-trading`，排除可变 Vite cache `frontend/node_modules/.vite`，修正 runtime `.venv` editable `.pth`，再由 `com.hyperalpha.ai-trading-local` 从 runtime 副本启动 frontend `5174`、backend `8802`、mock gateway `5621`，避免 macOS Documents 对后台执行的限制。
- LaunchAgent installer 曾因同步 `frontend/node_modules/.vite/deps/@monaco-editor_react.js` 这种临时 Vite cache 文件失败；`install_launch_agent.sh` 已排除该 cache，随后 `bash -n scripts/local-dev/install_launch_agent.sh`、`scripts/local-dev/install_launch_agent.sh` 和 `cd backend && uv run python scripts/ai_trading_v1_env_check.py --strict` 均通过。
...
- GitHub 上传按用户要求暂不处理；本地继续开发、测试、验收标记和提交。本轮未尝试 GitHub push。历史推送失败原因为 HTTPS 凭据不可读：`could not read Username for 'https://github.com': Device not configured`；本机也没有 `gh` CLI。
