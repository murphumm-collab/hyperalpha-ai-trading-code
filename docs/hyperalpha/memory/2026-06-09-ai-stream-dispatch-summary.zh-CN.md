...
- Hyper AI AI Trading panel 现在会同时读取 active/archived agent sessions；选中 session 后 specs/signals 列表加 `agent_session_id` 过滤并显示为 `Session specs` / `Session signals`，`Load session context` 会把 `/agent-sessions/{id}/context` 的 `ai_trading_agent_session_context.v1` 非敏感 packet 放进聊天输入，供 agent 做会话级复核。
...
- Agent-session history/context UI 后新增验证：`cd frontend && npm run build` 通过，`cd backend && uv run pytest tests/test_ai_trading_routes.py -q` 返回 31 条全绿；重跑 `scripts/local-dev/install_launch_agent.sh` 后 strict readiness 返回 `ready=true`；In-app Browser 复核可见 `Load session context`、`Session specs`、`Session signals`、本地归档 smoke row `Browser Archived Smoke`，点击 context 按钮后 textarea 包含 `ai_trading_agent_session_context.v1` packet。
