...
- Model-adjust session context 后新增验证：`cd backend && uv run python -m py_compile api/ai_trading_routes.py services/ai_trading_strategy_spec_service.py tests/test_ai_trading_routes.py` 通过；`cd backend && uv run pytest tests/test_ai_trading_routes.py -q` 返回 35 条全绿，覆盖未保存草案和已保存 spec 的敏感 session summary 都会被替换为 `[redacted_sensitive_context]` 且不进入 prompt/response，也覆盖未保存草案跨用户/archived session 在读取模型配置前失败；合并 AI Trading 回归返回 58 条全绿；`cd frontend && npm run build` 通过；`cd backend && uv run python scripts/ai_trading_v1_acceptance_smoke.py` 返回 `success=true`；一键本地 V1 验收 runner 最新证据为 spec `#26`、signal event `#24`、`agent_sessions.total=11`。
- Model-adjust session context 后已重跑 `scripts/local-dev/install_launch_agent.sh` 同步 runtime mirror；第一次 strict readiness 命中 backend 冷启动，12 秒后复查 `cd backend && uv run python scripts/ai_trading_v1_env_check.py --strict` 返回 `ready=true`；最新一键验收后 runtime totals 为 strategy specs `26`、signal events `24`、agent sessions `11`。
