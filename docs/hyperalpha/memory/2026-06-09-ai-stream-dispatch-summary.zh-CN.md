...
- Archived agent session 现在也会阻止已保存 strategy spec 继续 natural-language adjust 和 DeepSeek/Qwen model-adjust；model-adjust 在 archived gate 失败时不会读取模型配置或发模型请求，历史 detail/audit 仍可读。
