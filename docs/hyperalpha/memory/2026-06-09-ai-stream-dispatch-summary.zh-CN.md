...
- 新增一键本地 V1 验收 runner：`scripts/local-dev/run_ai_trading_v1_local_acceptance.sh --confirm-local-mock-handoff`，显式确认后才会跑 live mock handoff；已完整通过 backend compile、39 条 AI Trading 回归、API smoke、默认生产 gate 阻断、frontend build、runtime readiness、live local mock handoff，最新证据为 spec `#12`、signal event `#10`、runtime `target_kind=local_mock`；不带确认参数时已验证会拒绝运行并退出 2。
