...
| AI Trading agent-session detail page | Done | `/app/ai-trading/sessions/{agent_session_id}` renders a read-only current-user session audit page with context summary, specs, signals, blocker labels, refresh, and deterministic context compression |
