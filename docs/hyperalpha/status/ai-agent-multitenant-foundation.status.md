...
- Hyper AI exposes a Hyperliquid AI Trading focus strip using the user's watchlist or available symbols to prefill safe strategy prompts.
...
| Hyper AI trading focus UI | Done | Hyper AI config panel loads the current user's Hyperliquid watchlist, falls back to available symbols, and pre-fills no-auto-order strategy prompts per symbol |
...
- Passed: Hyper AI trading focus UI frontend build after adding the Hyperliquid watchlist/available-symbol prompt strip to the Agent config panel.
