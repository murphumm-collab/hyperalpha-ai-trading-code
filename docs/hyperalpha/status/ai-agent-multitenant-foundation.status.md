...
- Repeatable local live-stack mock handoff completed using `ai_trading_v1_live_stack_acceptance.py --confirm-local-mock-handoff`: latest evidence spec `#26`, signal event `#24`, gateway response summary `status=mock_accepted`, latest attempt `submitted`, runtime `target_kind=local_mock`, `agent_sessions.total=11`, and runtime model-adjustment blocker `model_profile_not_configured`.
