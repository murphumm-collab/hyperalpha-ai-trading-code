...
- `scripts/local-dev/run_ai_trading_v1_local_acceptance.sh --confirm-local-mock-handoff`：一键本地 V1 验收通过，覆盖 backend compile、82 条 AI Trading 回归、API-level smoke、live DeepSeek/Qwen model-adjust 默认阻断、默认生产 handoff gate 阻断、默认生产总 readiness gate 阻断、默认生产 DB-audit readiness gate 阻断、local V1 completion boundary audit、production completion boundary audit 预期阻断、production evidence template 预期阻断、frontend build、runtime readiness、live local mock handoff；最新证据为 spec `#63`、signal event `#61`、runtime `target_kind=local_mock`、`agent_sessions.total=48`、`handoff_attempts.total=59`、`model_adjustment.ready=false` 且 blocker 为 `model_profile_not_configured`。
...
- `cd backend && uv run pytest tests/test_ai_trading_v1_completion_audit.py tests/test_ai_trading_env_check.py tests/test_ai_trading_frontend_readiness_source.py tests/test_ai_trading_live_stack_acceptance.py tests/test_ai_trading_model_adjust_live_acceptance.py tests/test_ai_trading_production_readiness_check.py tests/test_ai_trading_production_readiness_api.py tests/test_ai_trading_routes.py tests/test_ai_trading_mock_gateway.py tests/test_ai_trading_production_handoff_check.py -q`：82 条 AI Trading/completion boundary/env checker/frontend readiness source guard/live-stack runner/live model-adjust runner/production readiness/admin readiness API/mock gateway/production handoff gate 回归通过，包含 production evidence artifact-ref-bounds/note-safety/path-safety/text-bounds/item-id/schema-field/text-quality/secret-pattern/artifact-ref/timestamp safety gate、completion audit、admin-only `handoff_audit` failed/blocked warning、agent-session context budget audit、agent-session latest over-budget/redacted/sensitive locator 和 locator next actions、Settings readiness locator UI source guard、production readiness CLI DB audit、model-adjust service context budget guard、runtime current-user agent-session context budget guard、env-check runtime agent context budget fail-closed gate、env-check precise `next_actions` 和 secret/summary redaction 覆盖。
...
- `cd backend && uv run python scripts/ai_trading_v1_env_check.py --strict`：在 Docker/Postgres/mock gateway/backend/frontend 启动后返回 `ready=true`，并结构化输出/校验 runtime gateway `target_kind=local_mock`、空 `runtime_config_blockers`、非敏感 `runtime_model_adjustment`、current-user `runtime_agent_context_budget.secret_policy=counts_only_no_summary_text`，以及 ready-state `next_actions=["Local AI Trading V1 runtime is ready; continue with browser acceptance or the aggregate V1 local acceptance runner."]`；缺少 `agent_sessions.context_budget` 或 secret policy 不等于 counts-only 时会 fail closed。
