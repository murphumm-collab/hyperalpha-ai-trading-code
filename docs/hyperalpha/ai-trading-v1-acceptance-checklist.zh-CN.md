...
- `cd backend && uv run python scripts/ai_trading_v1_env_check.py --strict`：在 Docker/Postgres/mock gateway/backend/frontend 启动后返回 `ready=true`，并结构化输出/校验 runtime gateway `target_kind=local_mock`、空 `runtime_config_blockers` 和非敏感 `runtime_model_adjustment`。
- 本轮 agent-session status payload 后已重跑 `scripts/local-dev/install_launch_agent.sh` 同步 runtime mirror；第一次 strict readiness 命中 backend 冷启动，重试 `cd backend && uv run python scripts/ai_trading_v1_env_check.py --strict` 返回 `ready=true`；最近一键验收后 runtime 显示 strategy specs `31`、signal events `29`、agent sessions `16`。
- `cd backend && uv run python scripts/ai_trading_v1_live_stack_acceptance.py --confirm-local-mock-handoff`：通过，覆盖本地 LaunchAgent/live Postgres 栈 draft -> save -> attach backtest -> approve -> eligible signal -> confirmed mock handoff -> attempt audit -> runtime；最新一键验收 live mock handoff 证据为 spec `#31`、signal event `#29`、gateway response `mock_accepted`。脚本默认拒绝无确认 handoff，只允许本地 URL，并要求 runtime gateway `target_kind=local_mock`。
...
- `cd frontend && npm run build`：通过，AI Trading runtime 摘要显示 Sessions 计数，Recent 区块显示 active/archived agent sessions，并能按当前 session 过滤 specs/signals；`agent_session_archived` blocker 在信号行、handoff title、session detail 和 attempt 审计文案中会显示为可读 `Agent session archived`；当前策略卡和 recent spec rows 会优先使用 record payload 中的 `agent_session.status` 判断归档状态，在 selected/owning session archived 时禁用 save/approve/adjust/model-adjust/backtest/signal-preview 等写动作，只保留 inspect/detail/audit 读操作；剩余为既有 browserslist/baseline/chunk-size warning。
