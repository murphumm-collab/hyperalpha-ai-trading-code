...
- `scripts/local-dev/run_ai_trading_v1_local_acceptance.sh --confirm-local-mock-handoff`：一键本地 V1 验收通过，覆盖 backend compile、63 条 AI Trading 回归、API-level smoke、live DeepSeek/Qwen model-adjust 默认阻断、默认生产 handoff gate 阻断、默认生产总 readiness gate 阻断、frontend build、runtime readiness、live local mock handoff；最新证据为 spec `#40`、signal event `#38`、runtime `target_kind=local_mock`、`agent_sessions.total=25`、`handoff_attempts.total=36`、`model_adjustment.ready=false` 且 blocker 为 `model_profile_not_configured`。
- `cd backend && uv run pytest tests/test_ai_trading_env_check.py tests/test_ai_trading_live_stack_acceptance.py tests/test_ai_trading_model_adjust_live_acceptance.py tests/test_ai_trading_production_readiness_check.py tests/test_ai_trading_production_readiness_api.py tests/test_ai_trading_routes.py tests/test_ai_trading_mock_gateway.py tests/test_ai_trading_production_handoff_check.py -q`：63 条 AI Trading/env checker/live-stack runner/live model-adjust runner/production readiness/admin readiness API/mock gateway/production handoff gate 回归通过，包含 admin-only `handoff_audit` failed/blocked warning、agent-session context budget audit、production readiness CLI DB audit、env-check precise `next_actions` 和 secret/summary redaction 覆盖。
...
- 本轮 production readiness CLI DB audit 后已重跑 `scripts/local-dev/install_launch_agent.sh` 同步 runtime mirror；首次 strict readiness 命中 backend 冷启动，约 12 秒后重试 `cd backend && uv run python scripts/ai_trading_v1_env_check.py --strict` 返回 `ready=true` 且不再输出误导性的启动服务动作；随后一键本地验收 runtime-after 显示 strategy specs `40`、signal events `38`、agent sessions `25`、handoff attempts `36`。
- `cd backend && uv run python scripts/ai_trading_v1_live_stack_acceptance.py --confirm-local-mock-handoff`：已由一键本地验收脚本复跑通过，覆盖本地 LaunchAgent/live Postgres 栈 draft -> save -> attach backtest -> approve -> eligible signal -> confirmed mock handoff -> attempt audit -> runtime；最新 live mock handoff 证据为 spec `#40`、signal event `#38`、gateway response `mock_accepted`。脚本默认拒绝无确认 handoff，只允许本地 URL，并要求 runtime gateway `target_kind=local_mock`。
