...
- 本轮 model-adjust session-context 后已重跑 `scripts/local-dev/install_launch_agent.sh` 同步 runtime mirror；首次 readiness 命中 backend 冷启动，12 秒后 `cd backend && uv run python scripts/ai_trading_v1_env_check.py --strict` 返回 `ready=true`；再次一键验收后 runtime 显示 strategy specs `26`、signal events `24`、agent sessions `11`。
- `cd backend && uv run python scripts/ai_trading_v1_live_stack_acceptance.py --confirm-local-mock-handoff`：通过，覆盖本地 LaunchAgent/live Postgres 栈 draft -> save -> attach backtest -> approve -> eligible signal -> confirmed mock handoff -> attempt audit -> runtime；最新证据为 spec `#26`、signal event `#24`、gateway response `mock_accepted`。脚本默认拒绝无确认 handoff，只允许本地 URL，并要求 runtime gateway `target_kind=local_mock`。
