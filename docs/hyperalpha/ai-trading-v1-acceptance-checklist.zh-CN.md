...
- `scripts/local-dev/run_ai_trading_v1_local_acceptance.sh --confirm-local-mock-handoff`：一键本地 V1 验收通过，覆盖 backend compile、76 条 AI Trading 回归、API-level smoke、live DeepSeek/Qwen model-adjust 默认阻断、默认生产 handoff gate 阻断、默认生产总 readiness gate 阻断、默认生产 DB-audit readiness gate 阻断、local V1 completion boundary audit、production completion boundary audit 预期阻断、production evidence template 预期阻断、frontend build、runtime readiness、live local mock handoff；最新证据为 spec `#54`、signal event `#52`、runtime `target_kind=local_mock`、`agent_sessions.total=39`、`handoff_attempts.total=50`、`model_adjustment.ready=false` 且 blocker 为 `model_profile_not_configured`。
...
- `cd backend && uv run pytest tests/test_ai_trading_v1_completion_audit.py tests/test_ai_trading_env_check.py tests/test_ai_trading_live_stack_acceptance.py tests/test_ai_trading_model_adjust_live_acceptance.py tests/test_ai_trading_production_readiness_check.py tests/test_ai_trading_production_readiness_api.py tests/test_ai_trading_routes.py tests/test_ai_trading_mock_gateway.py tests/test_ai_trading_production_handoff_check.py -q`：76 条 AI Trading/completion boundary/env checker/live-stack runner/live model-adjust runner/production readiness/admin readiness API/mock gateway/production handoff gate 回归通过，包含 production evidence note-safety/path-safety/text-bounds/item-id/schema-field/text-quality/secret-pattern/artifact-ref/timestamp safety gate、completion audit、admin-only `handoff_audit` failed/blocked warning、agent-session context budget audit、production readiness CLI DB audit、env-check precise `next_actions` 和 secret/summary redaction 覆盖。
...
- V1 完成度边界审计通过：`cd backend && uv run python scripts/ai_trading_v1_completion_audit.py --strict-local`；生产实盘切换审计 `--strict-production` 必须在真实外部验收前保持失败；真实外部验收材料必须按 `docs/hyperalpha/ai-trading-v1-production-evidence.template.json` 脱敏填写并通过 `--production-evidence-file` 校验，且填好的 evidence 文件必须放在代码仓库外或私有 ops evidence 位置，repo 内文件不能解锁 live ready；只能使用模板定义的 root/item 字段，`items` 只能包含模板列出的七个外部验收项，不能追加任意原始输出字段或未知验收项；root `notes` 必须是可选、短文本列表，不能塞入原始日志、模型输出、trace 或运维 dump；每项 `validated_by` / `evidence_summary` 必须是具体、非 placeholder 且长度受限的验收说明，不能塞入大段原始日志或模型输出；`generated_at` 与每项 `validated_at` 必须是带时区的 ISO-8601 时间戳，`generated_at` 不能早于任何 item `validated_at`，且 `artifact_refs` 必须非空，只能使用 `https://`、`ops://`、`lark://`、`notion://` 安全引用，不能嵌入 credentials、不能指向本机/私网/保留 IP。
- 后端 AI Trading 回归通过。
