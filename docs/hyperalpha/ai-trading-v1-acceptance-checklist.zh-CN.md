...
- `scripts/local-dev/run_ai_trading_v1_local_acceptance.sh --confirm-local-mock-handoff`：一键本地 V1 验收通过，覆盖 backend compile、70 条 AI Trading 回归、API-level smoke、live DeepSeek/Qwen model-adjust 默认阻断、默认生产 handoff gate 阻断、默认生产总 readiness gate 阻断、默认生产 DB-audit readiness gate 阻断、local V1 completion boundary audit、production completion boundary audit 预期阻断、production evidence template 预期阻断、frontend build、runtime readiness、live local mock handoff；最新证据为 spec `#46`、signal event `#44`、runtime `target_kind=local_mock`、`agent_sessions.total=31`、`handoff_attempts.total=42`、`model_adjustment.ready=false` 且 blocker 为 `model_profile_not_configured`。
...
- V1 完成度边界审计通过：`cd backend && uv run python scripts/ai_trading_v1_completion_audit.py --strict-local`；生产实盘切换审计 `--strict-production` 必须在真实外部验收前保持失败；真实外部验收材料必须按 `docs/hyperalpha/ai-trading-v1-production-evidence.template.json` 脱敏填写并通过 `--production-evidence-file` 校验，`generated_at` 与每项 `validated_at` 必须是 ISO-8601 时间戳，且 `artifact_refs` 必须非空，只能使用 `https://`、`ops://`、`lark://`、`notion://` 安全引用，不能嵌入 credentials、不能指向本机/私网/保留 IP。
- 后端 AI Trading 回归通过。
