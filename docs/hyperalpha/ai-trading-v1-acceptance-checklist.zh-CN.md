...
- 上下文压缩边界：agent session context packet 必须返回 requested/effective/max strategy/signal/attempt limits 和 `summary_max_chars`，压缩摘要必须受限并且 over-limit 请求必须被 API 拒绝，避免单用户长上下文拖垮共享 DeepSeek/Qwen 模型。
...
- `scripts/local-dev/run_ai_trading_v1_local_acceptance.sh --confirm-local-mock-handoff`：一键本地 V1 验收通过，覆盖 backend compile、59 条 AI Trading 回归、API-level smoke、live DeepSeek/Qwen model-adjust 默认阻断、默认生产 handoff gate 阻断、默认生产总 readiness gate 阻断、frontend build、runtime readiness、live local mock handoff；最新证据为 spec `#35`、signal event `#33`、runtime `target_kind=local_mock`、`agent_sessions.total=20`、`handoff_attempts.total=31`、`model_adjustment.ready=false` 且 blocker 为 `model_profile_not_configured`。
- `cd backend && uv run pytest tests/test_ai_trading_env_check.py tests/test_ai_trading_live_stack_acceptance.py tests/test_ai_trading_model_adjust_live_acceptance.py tests/test_ai_trading_production_readiness_check.py tests/test_ai_trading_production_readiness_api.py tests/test_ai_trading_routes.py tests/test_ai_trading_mock_gateway.py tests/test_ai_trading_production_handoff_check.py -q`：59 条 AI Trading/env checker/live-stack runner/live model-adjust runner/production readiness/admin readiness API/mock gateway/production handoff gate 回归通过，包含 admin-only `handoff_audit` failed/blocked warning 和 attempt secret redaction 覆盖。
...
- Agent session detail 页现在展示 `Context limits`、`returned / requested / max`、`Summary max chars 2000` 和 `redacted, no credentials`；In-app Browser 已打开 `/app/ai-trading/sessions/ait%3Abtc%3A4d53a40f6789` 验证该只读面板渲染且无 visible error。
...
- 本轮 admin readiness handoff audit 后已重跑 `scripts/local-dev/install_launch_agent.sh` 同步 runtime mirror；首次 strict readiness 碰到 backend 冷启动，重试后 `cd backend && uv run python scripts/ai_trading_v1_env_check.py --strict` 返回 `ready=true`；最近一键验收后 runtime 显示 strategy specs `35`、signal events `33`、agent sessions `20`、handoff attempts `31`。
- `cd backend && uv run python scripts/ai_trading_v1_live_stack_acceptance.py --confirm-local-mock-handoff`：通过，覆盖本地 LaunchAgent/live Postgres 栈 draft -> save -> attach backtest -> approve -> eligible signal -> confirmed mock handoff -> attempt audit -> runtime；最新一键验收 live mock handoff 证据为 spec `#35`、signal event `#33`、gateway response `mock_accepted`。脚本默认拒绝无确认 handoff，只允许本地 URL，并要求 runtime gateway `target_kind=local_mock`。
...
- Settings Admin AI Trading Production Readiness 面板已支持 `Handoff Audit` 可读组件名、failed/blocked handoff attempt warning 可读标签和 `Top Warnings` 区块；`cd frontend && npm run build` 通过，剩余为既有 browser-baseline/Browserslist/chunk-size warning。
